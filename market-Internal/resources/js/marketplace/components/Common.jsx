...
export function Empty({ title, action }) {
    return <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center"><h3 className="text-lg font-black">{title}</h3>{action ? <div className="mt-6">{action}</div> : null}</div>;
}
export function QueryError({ message = "Data belum dapat dimuat." }) {
    return <div className="rounded-3xl border border-rose-200 bg-rose-50 px-6 py-8 text-center text-rose-700"><h3 className="font-black">{message}</h3><button type="button" onClick={() => window.location.reload()} className="mt-4 rounded-xl bg-rose-700 px-5 py-2.5 font-bold text-white">Muat ulang</button></div>;
}
export function productPrice(product) {
    const minimum = Number(product?.price_min ?? product?.price ?? product?.default_variant?.price) || 0;
    const maximum = Number(product?.price_max ?? product?.price ?? product?.default_variant?.price) || minimum;
...
export function variantDescription(variant) {
    ...
    if (!attributes.length) {
        return variant.name === "Default" ? "Pilihan standar" : variant.name;
    }
    return attributes.map((item) => `${item.name}: ${item.value}`).join(" · ");
}
const statusClass = {
